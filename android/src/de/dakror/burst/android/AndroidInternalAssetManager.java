 ...
 * @author Dakror
 */
public class AndroidInternalAssetManager extends InternalAssetManager
{
	@Override
	public void init()
	{
		try
		{
			...
			{
				File parent = new File(AndroidInternalAssetManager.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile().getParentFile();
			...
			Gdx.app.log("AndroidInternalAssetManager.init", files + " files loaded.");
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public boolean isRunningFromJarFile()
	{
		try
		{
			return new File(AndroidInternalAssetManager.class.getProtectionDomain().getCodeSource().getLocation().toURI()).isFile();
		}
